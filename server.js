...
app.get(
  ...
  (req, res) => {
    ...
    db.query(sql, (err, results) => {
      ...
      console.log("ON envoie une bonne réponse");
      console.log(results);
      res.status(200).json(results);
    });
  }
...
app.post(
  ...
  (req, res) => {
    ...
    const sql = "INSERT INTO vehicles (id, brand, model, year) VALUES (?, ?, ?, ?)";
    db.query(sql, [id, brand, model, year], (err, result) => {
      if (err) {
        res.status(500).send("Server error");
        return;
      }
...
app.post(
  ...
  (req, res) => {
    const { id, brand, model, year } = req.body;
    const sql = "UPDATE vehicles SET brand = ?, model = ?, year = ? WHERE id = ?";
    db.query(sql, [brand, model, year, id], (err, result) => {
      if (err) {
        res.status(500).send("Server error");
        return;
      }
